...
class GetWorkout:
    ...
    def bmi_calc (self,name,height, weight): 
        ...
        self.user_information.update({name: {'BMI': BMI}})
    ...
    def notification(self):
        ...
        counter = 1
        user_hist_list = []
        workoutplan_list = list()
        result = pd.read_csv('workoutplan.csv')
        df = pd.DataFrame(result)
        workoutplan_list.append(df[f'Day {counter}'].dropna())
        user_hist_list.append(workoutplan_list)
        counter+=1
        ...
        Your Work Out of the Day(WOD) has been prepared by {self.trainer}.
